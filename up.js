...
//══════════[ ASUPAN MENU ]════════════════════════════//
             ...
             videoMsg = (await client.prepareMessageMedia(naura, "videoMessage", { thumbnail: naura, })).videoMessage
              ...
              break
              case 'emoji':
              if (!q) return reply(`mana emoji nya njing`)
              emoji = await getBuffer(`https://api-alphabot.herokuapp.com/api/emoji/apple?apikey=Alphabot&emoji=${c}`)
             await client.sendMessage(from, emoji, sticker, {quoted: mek})
              break
            case 'asupanbunga':
            if (isBanned) return reply(mess.banned)
            if (!isPremium) return reply(mess.only.premium)
...
case 'ph':
if (isBanned) return reply(mess.banned)
if (args.length < 1) return reply(`Nama yang mau di tulis?\nContoh: ${prefix + command} Zenix|404`)
reply(mess.wait)
hh = arg.split('|')[0]
hj = arg.split('|')[1]
bf = await getBuffer(`https://api-alphabot.herokuapp.com/api/textpro/pornhub?text=${hh}&text2=${hj}&apikey=${AlphaBot}`)
client.sendMessage(from, bf, image, { quoted: froxx, caption: 'Logo maker' })
break
case 'pipe':
if (isBanned) return reply(mess.banned)
...
teks = `${anu.display_url}`
suki = await getBuffer(`https://api-alphabot.herokuapp.com/api/image_editor/circle?apikey=Alphabot&url=${teks}`)
...
client.sendMessage(from, 'Waalaikumsalam, tumben dahh salam', text, {quoted: mek})
                  }
